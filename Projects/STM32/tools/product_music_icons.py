...
names=['arrow_downward','radio_button_unchecked','play_arrow','pause','skip_previous','skip_next']
...
static const uint8_t masks[PRODUCT_MUSIC_ICON_COUNT][288]={
'''
for blob in data:
    text+=' {\n'+ '\n'.join(','.join(f'0x{x:02x}' for x in blob[i:i+24])+',' for i in range(0,len(blob),24))+'\n },\n'
text+='};\nstatic const lv_image_dsc_t icons[PRODUCT_MUSIC_ICON_COUNT]={\n'
...
const lv_image_dsc_t *Product_MusicIcon(uint32_t mode)
{return mode<PRODUCT_MUSIC_ICON_COUNT?&icons[mode]:NULL;}
'''
from product_aux_assets import transform_icons
target=P/'Graphics/UI/src/product_music_icon_data.c';target.write_text(transform_icons(target.name,text),encoding='utf-8')
...
(A/'music-manifest.json').write_text(json.dumps(record,indent=2)+'\n')
