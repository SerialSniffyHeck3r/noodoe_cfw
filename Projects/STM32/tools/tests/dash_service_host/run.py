"""Production UART protocol/service C with mocked asynchronous hardware."""
...
incs=['Core/Inc','Middlewares/Noodoe/Vehicle/inc','Middlewares/Noodoe/Vehicle/src','Middlewares/Noodoe/Ambient/inc','Drivers/BSP/inc','Drivers/BSP/src','Drivers/STM32F4xx_HAL_Driver/Inc','Drivers/CMSIS/Device/ST/STM32F4xx/Include','Drivers/CMSIS/Include'];reports=[]
for optimization in ('O0','Os'):
    elf=out/f'dash-{optimization}.elf'
    cmd=[str(CC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11',f'-{optimization}','-Wall','-Wextra','-Werror','-DSTM32F429xx','-DUSE_HAL_DRIVER','-ffreestanding','-fno-builtin','-ffunction-sections','-fdata-sections','-nostdlib','-I'+str(HERE),*['-I'+str(PROJECT/p) for p in incs],str(HERE/'test.c'),'-Wl,-T,'+str(linker),'-Wl,--gc-sections','-Wl,-e,test_main','-o',str(elf),'-lgcc']
    ...
    emu.emu_start(symbols['test_main']|1,stop,timeout=10_000_000,count=10_000_000)
    word=lambda name:struct.unpack('<I',emu.mem_read(symbols[name],4))[0]
    report={'optimization':optimization,'returned':emu.reg_read(UC_ARM_REG_PC)==stop,'result':emu.reg_read(UC_ARM_REG_R0),'assertions':word('g_assertions'),'failure_line':word('g_failure_line'),'hardware_access':False,'source_sha256':hashlib.sha256((PROJECT/'Middlewares/Noodoe/Vehicle/src/DashService.c').read_bytes()).hexdigest()}
