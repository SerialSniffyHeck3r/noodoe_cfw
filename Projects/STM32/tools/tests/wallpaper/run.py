...
(OUT/'test.ld').write_text('MEMORY { FLASH(rx): ORIGIN = 0x10000000, LENGTH = 512K\n RAM(rwx): ORIGIN = 0x20000000, LENGTH = 256K }\n SECTIONS { .text : { KEEP(*(.text.test_*)) KEEP(*(.text.get_assertions)) *(.text*) *(.rodata*) } > FLASH\n .data : { *(.data*) } > RAM\n .bss (NOLOAD) : { *(.bss*) *(COMMON) } > RAM end = .; _end = .; }')
sources=[P/'App_Logic/UI/src/wallpaper.c',P/'App_Logic/UI/src/wallpaper_runtime.c',P/'Graphics/Port/src/graphics_background_math.c',P/'Graphics/Port/src/graphics_background.c',HERE/'test_stream.c']
sources += [P/'App_Logic/UI/src/ui_off_stages.c']
sources += [P/'Graphics/UI/src/scalar_transition.c',P/'Graphics/UI/src/page_transition.c',P/'Graphics/Port/src/graphics_background_clip.c',P/'Graphics/UI/src/speed_home_layout.c',HERE/'test_clip.c']
report=[]
for data_debug,opt in ((0,'-O0'),(0,'-Os'),(0,'-Oz'),(1,'-O0'),(1,'-Os'),(1,'-Oz')):
    elf=OUT/(opt[1:]+'-data'+str(data_debug)+'.elf')
    cmd=[str(TC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11',opt,'-ffunction-sections','-fdata-sections','-ffreestanding','-fno-builtin','-nostartfiles','--specs=nano.specs','--specs=nosys.specs','-Wall','-Wextra','-Werror','-I',str(P/'App_Logic/UI/inc'),*map(str,sources),str(HERE/'test_wallpaper.c'),'-I',str(P/'Graphics/UI/inc'),'-I',str(P/'Drivers/BSP/inc'),'-I',str(P/'Graphics/Port/inc'),'-T',str(OUT/'test.ld'),'-Wl,-e,test_policy','-Wl,--start-group','-lc','-lm','-lgcc','-lnosys','-Wl,--end-group','-o',str(elf)]
    cmd+=['-Wl,--gc-sections','-DLV_CONF_INCLUDE_SIMPLE','-DNOODOE_PRODUCT=1','-I',str(P/'Middlewares/Third_Party/LVGL')]
    cmd+=['-DDATA_DEBUG='+str(data_debug),'-I',str(P/'App_Logic/Config/inc')]
    cmd+=['-I',str(P/'Middlewares/Noodoe/Photos/inc')]
    if opt=='-Oz':
        names=set(re.findall(r'uint32_t (test_\w+)\(void\)', '\n'.join(x.read_text() for x in [HERE/'test_wallpaper.c',HERE/'test_stream.c',HERE/'test_clip.c'])))
