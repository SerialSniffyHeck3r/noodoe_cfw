...
incs=[HERE/'stubs',*list((PROJECT/'App_Logic').glob('*/inc')),*list((PROJECT/'Middlewares/Noodoe').glob('*/inc')),PROJECT/'App_Logic/Settings/src',PROJECT/'Drivers/BSP/inc',PROJECT/'Middlewares/Third_Party/FatFs/src'];reports=[]
for optimization in ('O0','Os'):
    elf=out/f'settings-{optimization}.elf'
    cmd=[str(CC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11',f'-{optimization}','-Wall','-Wextra','-Werror','-ffreestanding','-fno-builtin','-ffunction-sections','-fdata-sections','-nostdlib',*['-I'+str(p) for p in incs],str(PROJECT/'Drivers/BSP/src/noodoe_crc32.c'),str(HERE/'test_settings.c'),str(HERE/'test_usage.c'),str(PROJECT/'App_Logic/Vehicle/src/UsageCounter.c'),'-Wl,-T,'+str(linker),'-Wl,--gc-sections','-Wl,-e,Settings_TestMain','-o',str(elf),'-lgcc']
    ...
    emu=Uc(UC_ARCH_ARM,UC_MODE_THUMB|UC_MODE_MCLASS);emu.mem_map(0x10000000,0x10000);emu.mem_map(0x20000000,0x20000)
    ...
    emu.emu_start(symbols['Settings_TestMain']|1,stop,timeout=30_000_000,count=40_000_000)
    word=lambda name:struct.unpack('<I',emu.mem_read(symbols[name],4))[0]
    report={'optimization':optimization,'returned':emu.reg_read(UC_ARM_REG_PC)==stop,'result':emu.reg_read(UC_ARM_REG_R0),'assertions':word('g_assertions'),'failure_line':word('g_failure_line'),'mock_error':word('g_mock_error'),'hardware_access':False,'source_sha256':hashlib.sha256((PROJECT/'App_Logic/Settings/src/SettingsService.c').read_bytes()).hexdigest()}
