"""Build the isolated recovery APP with the installed CubeIDE ARM toolchain.
...
def build(out,stock):
    ...
    out=out.resolve();out.mkdir(parents=True,exist_ok=True);pack(stock,out)
    ...
    includes.add(P/'RecoveryGate/include')
    ...
    bsp=['bsp_boot.c','bsp_reset.s','bsp_fault.c','bsp_bringup.c','bsp_cpu_load.c','BSP_Buttons.c','BSP_Display.c','bsp_eve.c','bsp_eve_bus.c','bsp_lcd_panel.c','bsp_backlight.c','BSP_NOR.c','BSP_Power.c','BSP_RAM.c','bsp_bt_hci.c','BSP_BT_ResetDiagnostic.c','BSP_Watchdog.c','BSP_Ambient.c']
    ...
    for name in ('Bootstrap','Recovery'):
        sources += list((P/'App_Logic'/name/'src').glob('*.c'))
    sources.append(P/'App_Logic/Runtime/src/RuntimeUpdate.c')
    for name in ('Bluetooth','Recovery','Health'):
        sources += list((P/'Middlewares/Noodoe'/name/'src').glob('*.c'))
    sources += [P/'Drivers/BSP/src/noodoe_crc32.c',P/'Middlewares/Noodoe/Ambient/src/AmbientService.c']
    for name in ('Update_SHA256.c','Update_Metadata.c','Update_Service.c','NDCP.c','Recovery_Core.c'):
        sources.append(P/'Middlewares/Noodoe/Update/src'/name)
    sources.append(P/'Middlewares/Noodoe/Resources/src/Resources_Format.c')
    sources.append(P/'Middlewares/Noodoe/StorageSWD/src/StorageSWD.c')
    for name in ('recovery_store.c','bootstrap_storage.c','bootstrap_resources.c','bootstrap_bond.c','gate_format_shared.c','event_log_shared.c'):
        f=P/'Middlewares/Noodoe/Storage/src'/name
    ...
    sources += [P/'Middlewares/Third_Party/miniz/miniz_tinfl.c',out/'stock_asset.s',P/'Graphics/UI/src/bootstrap_screen.c',P/'RecoveryGate/src/gate_board.c',P/'RecoveryGate/src/gate_display.c']
    defs=['USE_HAL_DRIVER','STM32F429xx','NOODOE_BOOTSTRAP=1','NOODOE_PRODUCT=0','NOODOE_INTEGRATED=0','configAPPLICATION_ALLOCATED_HEAP=1','MINIZ_NO_ARCHIVE_APIS','MINIZ_NO_DEFLATE_APIS','MINIZ_NO_STDIO','MINIZ_NO_TIME','MINIZ_NO_MALLOC','MINIZ_NO_ZLIB_APIS','NDEBUG']
    ...
    elf=out/'bootstrap.elf'
    cmd=[str(TC/'arm-none-eabi-gcc.exe'),*flags,'--specs=nosys.specs',*map(str,objects),'-Wl,-T,'+str(P/'Linker/Noodoe_APP.ld'),'-Wl,--gc-sections','-Wl,-Map,'+str(out/'bootstrap.map'),'-Wl,--print-memory-usage','-o',str(elf),'-lm']
    r=subprocess.run(cmd,capture_output=True,text=True);(out/'link.log').write_text(r.stdout+r.stderr)
    if r.returncode:raise RuntimeError(r.stdout+r.stderr)
    subprocess.run([str(TC/'arm-none-eabi-objcopy.exe'),'-O','binary','--gap-fill=0xFF',str(elf),str(out/'bootstrap.bin')],check=True)
    image=(out/'bootstrap.bin').read_bytes()
    if not 0x50000<len(image)<=0x70000:raise ValueError('Stock OTA size gate not satisfied')
    result=dict(bytes=len(image),free_bytes=0x70000-len(image),sha256=hashlib.sha256(image).hexdigest(),sources=len(sources),profile='Bootstrap',wireless_verified=False)
