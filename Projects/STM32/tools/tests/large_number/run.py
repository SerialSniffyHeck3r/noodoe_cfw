"""Run real enlarged-number draw/evaluation and emitted bounds on Cortex-M4."""
...
parts=['static const lv_font_glyph_dsc_t metrics[128]={']
for i,g in enumerate(fonts['product_number_64']['glyphs']):
 w,h,x,y=g['box'];parts.append(f"[{g['codepoint']}]={{.box_w={w},.box_h={h},.ofs_x={x},.ofs_y={y},.gid.index={i}}},")
(out/'metrics.h').write_text('\n'.join(parts+['};']))
...
for opt in ('O0','Os'):
 ...
 cmd=[str(TC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11','-'+opt,'-ffunction-sections','-fdata-sections','-ffreestanding','-nostartfiles','--specs=nano.specs','--specs=nosys.specs','-DLV_CONF_INCLUDE_SIMPLE','-DNOODOE_PRODUCT=1','-DNOODOE_INTEGRATED=1','-I'+str(P/'Graphics/Port/inc'),'-I'+str(lv),str(P/'Graphics/Port/src/graphics_large_number.c'),str(H/'test_number.c'),'-T',str(out/'test.ld'),'-Wl,-e,test_number','-Wl,--gc-sections','-lc','-lgcc','-o',str(elf)]
 r=subprocess.run(cmd,capture_output=True,text=True);(out/(opt+'-compile.log')).write_text(r.stdout+r.stderr);assert r.returncode==0,r.stdout+r.stderr
 ...
 u.emu_start(syms['test_number']|1,0x1003fff0,count=20000000)
 ...
 report={'optimization':opt,'speed_cases':401,'assertions':struct.unpack('<I',u.mem_read(syms['assertions'],4))[0],'status':'PASS','hardware_access':False};reports.append(report);print(json.dumps(report),flush=True)
