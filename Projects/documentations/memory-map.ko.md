...
조사한 보드는 128MiB NOR와 64MiB SDRAM을 쓴다. NOR는 전원이 꺼져도 남고, SDRAM은 작업 메모리다. 순정 NOR에는 FAT 파일과 업데이트용 예약 공간이 있다. CFW는 빈 클러스터의 소유권, FAT 사본, 예약 영역과의 충돌을 확인한 후 **이름이 있는 고정 크기 파일**을 만든다.  NOR 오프셋 0x07F80000–0x07FFFFFF는 순정 업데이터를 위해 보존한다.
