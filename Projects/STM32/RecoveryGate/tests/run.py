...
a=argparse.ArgumentParser();a.add_argument('--all-cuts',action='store_true');a.add_argument('--policy-only',action='store_true');a.add_argument('--output',type=Path);args=a.parse_args()
out=args.output or HERE/'output';out.mkdir(parents=True,exist_ok=True)
...
sources=[HERE/'test_gate.c',*[P/'RecoveryGate/src'/f for f in ('gate_policy.c','gate_format.c','gate_engine.c','event_log.c')],P/'Middlewares/Noodoe/Update/src/Update_SHA256.c']
reports=[]
for opt in ('O0','Os'):
 elf=out/(opt+'.elf');cmd=[str(TC/'arm-none-eabi-gcc.exe'),str(next(q for q in Path(__file__).resolve().parents if (q/'tools/build.ps1').is_file())/'Drivers/BSP/src/noodoe_crc32.c'),'-I'+str(next(q for q in Path(__file__).resolve().parents if (q/'tools/build.ps1').is_file())/'Drivers/BSP/inc'),'-mcpu=cortex-m4','-mthumb','-std=c11','-'+opt,'-Wall','-Wextra','-Wno-misleading-indentation','-ffreestanding','-fno-builtin','-nostdlib','-I'+str(P/'RecoveryGate/include'),'-I'+str(P/'Middlewares/Noodoe/Update/inc'),*map(str,sources),'-T',str(ld),'-Wl,-e,TestPolicy','-lgcc','-o',str(elf)]
 ...
 cases=[('TestPolicy',0)]
 if opt=='Os' and not args.policy_only:cases += [('TestEngine',mode)for mode in (list(range(104)) if args.all_cuts else [0,1,2,3,4,5,52,100,101,102,103])]
 if opt=='Os' and not args.policy_only:cases += [('TestDiagnostic',mode) for mode in [0,1,2,52,100,101,102,103]]
 for entry,mode in cases:
  ...
  source=bytearray((i*37+11)&255 for i in range(0x60000));struct.pack_into('<II',source,0,0x2002ff00,0x08020401);struct.pack_into('<III',source,0x200,0x51534352,1,1);struct.pack_into('<4I',source,0x230,0x3250554e,2,2,0x60000);u.mem_write(0x11000000,bytes(source))
