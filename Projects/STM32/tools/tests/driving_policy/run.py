...
sources=[P/f for f in ['App_Logic/UI/src/ignition_session.c','Middlewares/Noodoe/Input/src/InputMode.c','App_Logic/UI/src/phone_trail.c','App_Logic/UI/src/wallpaper.c','Graphics/Port/src/graphics_background_math.c','Middlewares/Noodoe/Ambient/src/BacklightPolicy.c']]
report=[]
for data_debug,opt in ((0,'-O0'),(0,'-Os'),(0,'-Oz')):
    elf=OUT/(opt[1:]+'-data'+str(data_debug)+'.elf')
    cmd=[str(TC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mfpu=fpv4-sp-d16','-mfloat-abi=hard','-mthumb','-std=c11',opt,'-ffreestanding','-fno-builtin','-nostartfiles','--specs=nano.specs','--specs=nosys.specs','-Wall','-Wextra','-Werror','-I',str(P/'App_Logic/UI/inc'),*map(str,sources),str(HERE/'test.c'),'-I',str(P/'Graphics/UI/inc'),'-I',str(P/'Drivers/BSP/inc'),'-I',str(P/'Graphics/Port/inc'),'-T',str(OUT/'test.ld'),'-Wl,-e,test_input','-Wl,--start-group','-lc','-lm','-lgcc','-lnosys','-Wl,--end-group','-o',str(elf)]
    cmd+=['-I'+str(P/'Middlewares/Noodoe/Input/inc'),'-I'+str(P/'Middlewares/Noodoe/Ambient/inc'),'-DDATA_DEBUG='+str(data_debug),'-I',str(P/'App_Logic/Config/inc')]
    if opt=='-Oz':
        names=set(re.findall(r'uint32_t (test_\w+)\(void\)',(HERE/'test.c').read_text()))
