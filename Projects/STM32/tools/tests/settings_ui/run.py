...
for optimization in ('O0','Os'):
    ...
    cmd=[str(CC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11',f'-{optimization}','-Wall','-Wextra','-Werror','-ffreestanding','-fno-builtin','-ffunction-sections','-fdata-sections','-nostartfiles','--specs=nano.specs','--specs=nosys.specs',*['-I'+str(p) for p in incs],str(HERE/'test_settings_ui.c'),*map(str,[PROJECT/f for f in ['Middlewares/Noodoe/Ambient/src/BacklightPolicy.c','App_Logic/Settings/src/app_settings.c', 'App_Logic/Settings/src/settings_catalog.c', 'App_Logic/Settings/src/settings_display.c', 'App_Logic/Settings/src/settings_time.c', 'App_Logic/Settings/src/settings_vehicle.c', 'App_Logic/Settings/src/settings_maintenance.c', 'App_Logic/Settings/src/settings_debug.c', 'App_Logic/Settings/src/settings_connections.c', 'App_Logic/Settings/src/settings_system.c', 'App_Logic/Settings/src/settings_power.c', 'App_Logic/Settings/src/settings_motion.c', 'App_Logic/Settings/src/settings_ui.c', 'Drivers/BSP/src/BSP_Calendar.c', 'App_Logic/UI/src/ui_maintenance.c', 'Graphics/UI/src/page_transition.c', 'Graphics/UI/src/scene_transition.c','Graphics/UI/src/power_scene.c']]),'-Wl,-T,'+str(linker),'-Wl,--gc-sections','-Wl,-e,SettingsUI_TestMain','-o',str(elf),'-lgcc']
    ...
    emu.emu_start(symbols['SettingsUI_TestMain']|1,stop,timeout=30_000_000,count=40_000_000)
    word=lambda name:struct.unpack('<I',emu.mem_read(symbols[name],4))[0]
    report={'optimization':optimization,'returned':emu.reg_read(UC_ARM_REG_PC)==stop,'result':emu.reg_read(UC_ARM_REG_R0),'assertions':word('g_assertions'),'failure_line':word('g_failure_line'),'hardware_access':False,'source_sha256':hashlib.sha256((PROJECT/'App_Logic/Settings/src/settings_ui.c').read_bytes()).hexdigest()}
