"""Compile actual NOR BSP against installed HAL headers; emulate policy only."""
...
sources=[HERE/'test_gate.c',PROJECT/'Drivers/BSP/src/BSP_NOR.c'];reports=[]
incs=['Core/Inc','Drivers/BSP/inc','Drivers/STM32F4xx_HAL_Driver/Inc','Drivers/CMSIS/Device/ST/STM32F4xx/Include','Drivers/CMSIS/Include','Middlewares/Third_Party/FreeRTOS/Source/CMSIS_RTOS_V2']
for optimization in ('O0','Os'):
    elf=out/f'gate-{optimization}.elf'
    cmd=[str(CC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11',f'-{optimization}','-Wall','-Wextra','-Werror','-DSTM32F429xx','-DUSE_HAL_DRIVER','-ffreestanding','-fno-builtin','-ffunction-sections','-fdata-sections','-nostdlib',*['-I'+str(PROJECT/p) for p in incs],*map(str,sources),'-Wl,-T,'+str(linker),'-Wl,--gc-sections','-Wl,-e,NorGate_TestMain','-o',str(elf),'-lgcc']
    ...
    for address,size in ((0x10000000,0x10000),(0x20000000,0x20000),(0x08008000,0x1000),(0x40015000,0x1000)):
    ...
    emu.emu_start(symbols['NorGate_TestMain']|1,stop,timeout=20_000_000,count=20_000_000)
