"""Actual Control + NDCP C at O0/Os; Bluetooth/Runtime/Update boundary mocks."""
...
sources=[HERE/'test_control.c',PROJECT/'App_Logic/Control/src/NoodoeControl.c',PROJECT/'App_Logic/Control/src/MediaControl.c',PROJECT/'Middlewares/Noodoe/Update/src/NDCP.c'];reports=[]
for optimization in ('O0','Os'):
    elf=out/f'control-{optimization}.elf';incs=[HERE/'stubs',PROJECT/'Drivers/BSP/inc',*list((PROJECT/'App_Logic').glob('*/inc')),*list((PROJECT/'Middlewares/Noodoe').glob('*/inc'))]
    cmd=[str(CC/'arm-none-eabi-gcc.exe'),str(next(q for q in Path(__file__).resolve().parents if (q/'tools/build.ps1').is_file())/'Drivers/BSP/src/noodoe_crc32.c'),'-I'+str(next(q for q in Path(__file__).resolve().parents if (q/'tools/build.ps1').is_file())/'Drivers/BSP/inc'),'-mcpu=cortex-m4','-mthumb','-std=c11',f'-{optimization}','-Wall','-Wextra','-Werror','-ffreestanding','-fno-builtin','-ffunction-sections','-fdata-sections','-nostdlib',*['-I'+str(p) for p in incs],*map(str,sources),'-Wl,-T,'+str(linker),'-Wl,--gc-sections','-Wl,-e,Control_TestMain','-o',str(elf),'-lgcc']
    ...
    emu=Uc(UC_ARCH_ARM,UC_MODE_THUMB|UC_MODE_MCLASS);emu.mem_map(0x10000000,0x10000);emu.mem_map(0x20000000,0x20000);emu.mem_map(0x08008000,0x1000);emu.mem_write(0x08008000,struct.pack('<5I',0xE0000,0,0,0,0))
    ...
    emu.emu_start(symbols['Control_TestMain']|1,stop,timeout=20_000_000,count=20_000_000)
    word=lambda name:struct.unpack('<I',emu.mem_read(symbols[name],4))[0]
    report={'optimization':optimization,'returned':emu.reg_read(UC_ARM_REG_PC)==stop,'result':emu.reg_read(UC_ARM_REG_R0),'assertions':word('g_assertions'),'failure_line':word('g_failure_line'),'mock_error':word('g_mock_error'),'hardware_access':False,'source_sha256':hashlib.sha256(sources[1].read_bytes()).hexdigest()}
