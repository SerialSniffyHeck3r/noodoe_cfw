...
/* Noodoe project configuration for unmodified LVGL v9.5.0.
 * The graphics task alone owns LVGL. FreeRTOS remains outside LVGL's OS layer.
 * LVGL's 64 KiB builtin heap is separate from the FreeRTOS heap; no SDRAM is used.
 * EVE generation 2 selects the FT81x family, not a proven FT812/FT813 subtype.
 * No software fallback exists: unsupported EVE effects must be rejected/removed
 * by the Graphics layer instead of allocating a CPU framebuffer implicitly.
 * This file is outside Cube-generated Core and the immutable vendor source.
 */
#ifndef __ASSEMBLY__
#include <stdint.h>
#ifdef __cplusplus
extern "C" {
#endif
/* Declare only this hook here: including Graphics.h would recurse via lvgl.h. */
extern void Graphics_AssertFail(const char *file, uint32_t line) __attribute__((noreturn));
#ifdef __cplusplus
}
#endif
#endif
 ...
 * - LV_STDLIB_CUSTOM:      Implement the functions externally
 */
/* Reuse the same bounded newlib-nano formatter as application code. The
 * allocator/string choices stay unchanged; this does not move the LVGL heap. */
...
#define LV_USE_LOG 1
    ...
    /* The vendor logger prints the basename anyway, but normally stores the
     * complete source path before stripping it at runtime. Its supported log
     * macro overrides let the compiler store only that same basename. Retain
     * severity, message, function, line and timestamp in both build profiles. */
    #ifdef __FILE_NAME__
    #define LV_LOG_WARN(...) do { if(LV_LOG_LEVEL <= LV_LOG_LEVEL_WARN) lv_log_add(LV_LOG_LEVEL_WARN, __FILE_NAME__, __LINE__, __func__, __VA_ARGS__); } while(0)
    #define LV_LOG_ERROR(...) do { if(LV_LOG_LEVEL <= LV_LOG_LEVEL_ERROR) lv_log_add(LV_LOG_LEVEL_ERROR, __FILE_NAME__, __LINE__, __func__, __VA_ARGS__); } while(0)
    #define LV_LOG_USER(...) do { if(LV_LOG_LEVEL <= LV_LOG_LEVEL_USER) lv_log_add(LV_LOG_LEVEL_USER, __FILE_NAME__, __LINE__, __func__, __VA_ARGS__); } while(0)
    #endif
...
/* lv_assert.h expands this as a complete statement without adding ';'. */
/* Keep the useful source filename + line in diagnostics, without repeating
 * the long vendor directory prefix at every assertion site in APP flash. */
#ifdef __FILE_NAME__
#define LV_ASSERT_HANDLER Graphics_AssertFail(__FILE_NAME__, __LINE__);
#else
#define LV_ASSERT_HANDLER Graphics_AssertFail(__FILE__, __LINE__);
#endif
 ...
 */
#if NOODOE_PRODUCT
#define LV_FONT_CUSTOM_DECLARE LV_FONT_DECLARE(product_text_16)
#else
#define LV_FONT_CUSTOM_DECLARE
#endif

/** Always set a default font */
/* Four 4-bpp fonts are built; no CJK or compressed 3-bpp font is selected.
 * If the final 448 KiB APP image overflows, remove 40 then 28 after updating UI
 * users; font selection is a flash budget decision, not a silent runtime swap. */
#if NOODOE_PRODUCT
/* ProductBoot binds Lato before lv_init. All visible Product labels already
 * select Lato/D-DIN explicitly; use that same family for LVGL's default style
 * instead of retaining an unused second font's metrics and kerning tables.
 * Other test profiles retain Montserrat and its existing demo sizes. */
#define LV_FONT_DEFAULT &product_text_16
#else
