...
incs=[H.parent/'stock_photos/stubs',P/'Drivers/BSP/inc',P/'Middlewares/Noodoe/Storage/inc',P/'Middlewares/Noodoe/Photos/inc',P/'Middlewares/Third_Party/FatFs/src',P/'Middlewares/Third_Party/LVGL/src/libs/tjpgd']
...
backup=P.parents[1]/'analysis/2026-09-16-home-power-wallpaper/nor-current-reconstructed.bin'
reports=[]
for opt in ['O0','Os','Oz']:
 elf=O/(opt+'.elf')
 ...
 files=[P.parents[1]/'analysis/2026-09-16-home-power-wallpaper/darkimagetest_1.jpg']
 lengths=[]
 ...
 u.mem_write(symbols['input_lengths'],struct.pack('<2I',lengths[0],0))
 ...
 if opt=='Os':
  ...
  oldfs=Fat(before);newfs=Fat(media)
  # Every preexisting file byte/chain remains untouched, including NOODOE.RSC.
  assert set(newfs.files)==set(oldfs.files)|{'WALL0.JPG'}
  for name,(chain,n) in oldfs.files.items():
   assert newfs.files[name]==(chain,n),name
   for cluster in chain:
    at=oldfs.address(cluster);assert before[at:at+32768]==media[at:at+32768],name
  chain,n=newfs.files['WALL0.JPG'];assert n==files[0].stat().st_size
  for cluster in chain:
   assert oldfs.fat(cluster)==0 and cluster not in oldfs.refs
   at=newfs.address(cluster);assert at+32768<=0x7f70000
   watched.update(range(at,at+32768,4096))
  assert before[0x7f70000:]==media[0x7f70000:]
  (O/'after.bin').write_bytes(media)
  ...
  plan={'schema':2,'backup_sha256':hashlib.sha256(before).hexdigest(),'source_provenance':json.loads((backup.parent/'nor-provenance.json').read_text()),'uid':[3735583,875974927,892810041],
        'regions':regions,'images':[{'slot':i|0x100,'path':str(f),'sha256':hashlib.sha256(f.read_bytes()).hexdigest()} for i,f in enumerate(files)]}
  ...
  address,w,h,n,revision=struct.unpack('<5I',u.mem_read(symbols['result_images'],20));(O/'new-photo.rgb565').write_bytes(bytes(u.mem_read(address,n)))
