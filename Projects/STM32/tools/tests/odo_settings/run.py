...
for optimization in ('O0','Os'):
    ...
    sources=['App_Logic/Vehicle/src/odometer_guard.c','App_Logic/Settings/src/settings_motion.c']
    cmd=[str(CC/'arm-none-eabi-gcc.exe'),'-mcpu=cortex-m4','-mthumb','-std=c11','-DNOODOE_PRODUCT=1',f'-{optimization}','-Wall','-ffunction-sections','-fdata-sections','-nostartfiles','--specs=nano.specs','--specs=nosys.specs',*['-I'+str(p) for p in incs],str(HERE/'test.c'),*[str(PROJECT/f) for f in sources],'-Wl,-T,'+str(linker),'-Wl,--gc-sections','-Wl,-e,Test','-o',str(elf),'-lgcc']
    ...
    report={'optimization':optimization,'returned':emu.reg_read(UC_ARM_REG_PC)==stop,'result':emu.reg_read(UC_ARM_REG_R0),'assertions':word('assertions'),'failure_line':emu.reg_read(UC_ARM_REG_R0),'hardware_access':False,'source_sha256':{name:hashlib.sha256((PROJECT/name).read_bytes()).hexdigest() for name in sources+['App_Logic/Vehicle/src/odometer_service.c']}}
