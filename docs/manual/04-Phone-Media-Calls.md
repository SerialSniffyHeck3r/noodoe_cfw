...
맨 위 한 줄에는 휴대전화 아이콘·이름·배터리 %가 표시됩니다. 알림이 없으면 `No notification`을 표시하고, 알림이 있으면 첫 화면부터 최신 알림을 표시합니다. UP/DOWN으로 최근 알림 1~10개를 순환합니다. 앱 아이콘과 앱 이름, 제목, 본문 순서입니다. **최대 10개**를 보관하며 동일 알림의 수정/삭제를 반영합니다.
