"""Build the eight fixed navigation masks from pinned Material Icons Round.
...
names=['home','analytics','smartphone','music_note','car_repair','settings_remote','route','settings']
...
text+='};\nstatic const lv_image_dsc_t icons[PRODUCT_MODE_ICON_COUNT]={\n'
for n in range(8):text+=f' {{.header={{.magic=LV_IMAGE_HEADER_MAGIC,.cf=LV_COLOR_FORMAT_A4,.w=24,.h=24,.stride=12}},.data_size=288,.data=masks[{n}]}},\n'
text+='''};
...
target=P/'Graphics/UI/src/product_mode_icon_data.c';target.write_text(transform_icons(target.name,text),encoding='utf-8')
...
(A/'mode-manifest.json').write_text(json.dumps(record,indent=2)+'\n')
